"use client";

import { AlertCircle, Check, Eye, Sparkles, WandSparkles, X } from "lucide-react";
import type { DraftGenerationAction, GeneratedDraft, GenerationPreset, GenerationPresetKind } from "@/lib/api/index";
import { draftActionLabels, presetSaveLabels, sourceLabels } from "../../constants";
import type { PresetSaveState } from "../../types";
import { PresetEditor } from "./PresetEditor";

export function GenerationDialog({
  action,
  authorPreset,
  authorPresets,
  error,
  isGenerating,
  isPreviewing,
  onAccept,
  onAddPreset,
  onChangeAuthorPreset,
  onChangePresetContent,
  onChangeWritingPreset,
  onClose,
  onDeletePreset,
  onDiscard,
  onGenerate,
  onPreview,
  onRenamePreset,
  presetSaveState,
  result,
  selectedAuthorPresetId,
  selectedWritingPresetId,
  writingPreset,
  writingPresets,
}: {
  action: DraftGenerationAction;
  authorPreset: GenerationPreset | undefined;
  authorPresets: GenerationPreset[];
  error: string | null;
  isGenerating: boolean;
  isPreviewing: boolean;
  onAccept: () => void;
  onAddPreset: (kind: GenerationPresetKind) => void;
  onChangeAuthorPreset: (presetId: string) => void;
  onChangePresetContent: (
    kind: GenerationPresetKind,
    preset: GenerationPreset,
    contentValue: string,
  ) => void;
  onChangeWritingPreset: (presetId: string) => void;
  onClose: () => void;
  onDeletePreset: (preset: GenerationPreset) => void;
  onDiscard: () => void;
  onGenerate: () => void;
  onPreview: () => void;
  onRenamePreset: (preset: GenerationPreset) => void;
  presetSaveState: PresetSaveState;
  result: GeneratedDraft | null;
  selectedAuthorPresetId: string;
  selectedWritingPresetId: string;
  writingPreset: GenerationPreset | undefined;
  writingPresets: GenerationPreset[];
}) {
  const canGenerate = Boolean(writingPreset && authorPreset && !isGenerating);

  return (
    <div className="modal-backdrop generation-backdrop" role="presentation">
      <section
        aria-label={draftActionLabels[action]}
        className="settings-dialog generation-dialog"
        role="dialog"
      >
        <header className="settings-heading">
          <div>
            <p className="eyebrow">Draft Generator</p>
            <h2>{draftActionLabels[action]}</h2>
          </div>
          <button
            aria-label="关闭"
            className="icon-button"
            disabled={isGenerating || Boolean(result)}
            onClick={onClose}
            type="button"
          >
            <X size={18} />
          </button>
        </header>

        <PresetEditor
          kind="writing_mode"
          label="写作方式"
          onAddPreset={onAddPreset}
          onChangePreset={onChangeWritingPreset}
          onChangePresetContent={onChangePresetContent}
          onDeletePreset={onDeletePreset}
          onRenamePreset={onRenamePreset}
          placeholder="选择写作方式后可直接修改提示词。"
          preset={writingPreset}
          presets={writingPresets}
          selectedPresetId={selectedWritingPresetId}
        />

        <PresetEditor
          kind="author_persona"
          label="执笔作者人格"
          onAddPreset={onAddPreset}
          onChangePreset={onChangeAuthorPreset}
          onChangePresetContent={onChangePresetContent}
          onDeletePreset={onDeletePreset}
          onRenamePreset={onRenamePreset}
          placeholder="填写作者人格或人格设定 Skill。"
          preset={authorPreset}
          presets={authorPresets}
          selectedPresetId={selectedAuthorPresetId}
        />

        {presetSaveState !== "idle" ? (
          <div className={`preset-save-hint ${presetSaveState}`}>
            <Check size={14} />
            {presetSaveLabels[presetSaveState]}
          </div>
        ) : null}

        {error ? (
          <div className="generation-error" role="alert">
            <AlertCircle size={15} />
            <span>{error}</span>
          </div>
        ) : null}

        {result ? (
          <section className="generated-draft visible" aria-label="生成结果">
            <div className="generated-draft-meta">
              <Sparkles size={14} />
              <span>
                {sourceLabels[result.source]} · {result.model ?? "本地兜底"}
              </span>
            </div>
            <div className="generated-draft-text">{result.text}</div>
          </section>
        ) : null}

        {result ? (
          <div className="dialog-actions">
            <button className="secondary-button" onClick={onDiscard} type="button">
              废弃
            </button>
            <button className="primary-button" onClick={onAccept} type="button">
              <Check size={16} />
              采纳
            </button>
          </div>
        ) : (
          <div className="dialog-actions">
            <button
              className="secondary-button"
              disabled={isGenerating}
              onClick={onClose}
              type="button"
            >
              取消
            </button>
            <button
              className="secondary-button"
              disabled={isPreviewing || isGenerating}
              onClick={onPreview}
              type="button"
            >
              <Eye size={16} />
              {isPreviewing ? "预览中" : "预览"}
            </button>
            <button
              className="primary-button"
              disabled={!canGenerate}
              onClick={onGenerate}
              type="button"
            >
              <WandSparkles size={16} />
              {isGenerating ? "生成中" : "生成"}
            </button>
          </div>
        )}
      </section>
    </div>
  );
}
